import { Client, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://appwrite.grovehub.com.br/v1')
    .setProject('69b19b99002f199c90e2')
    .setKey('standard_');

const databases = new Databases(client);

async function checkAttributes() {
    const collections = ['contacts', 'campaigns', 'templates'];
    for (const colId of collections) {
        try {
            const col = await databases.getCollection('default', colId);
            console.log(`\nColeção: ${colId} (${col.name})`);
            const attributes = await databases.listAttributes('default', colId);
            attributes.attributes.forEach(attr => {
                console.log(`- ${attr.key} (${attr.type}): status=${attr.status}`);
            });
        } catch (e) {
            console.error(`Erro ao checar ${colId}:`, e.message);
        }
    }
}

checkAttributes();
