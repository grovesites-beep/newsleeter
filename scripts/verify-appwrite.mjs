import { Client, Functions, Databases } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://appwrite.grovehub.com.br/v1')
    .setProject('69b19b99002f199c90e2')
    .setKey('standard_');

const functions = new Functions(client);
const databases = new Databases(client);

async function verify() {
    console.log('--- Verificação Final ---');
    try {
        const funcs = await functions.list();
        console.log('Funções encontradas:', funcs.functions.length);
        funcs.functions.forEach(f => {
            console.log(`- Nome: ${f.name} | ID: ${f.$id} | Runtime: ${f.runtime}`);
        });

        const collections = await databases.listCollections('default');
        console.log('Coleções no banco "default":', collections.total);
        collections.collections.forEach(c => {
            console.log(`- ${c.name} (${c.$id})`);
        });
    } catch (e) {
        console.error('Erro na verificação:', e.message);
    }
}

verify();
